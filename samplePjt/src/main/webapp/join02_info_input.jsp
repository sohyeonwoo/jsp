...
<html>
	<head>
		...
		//	var idPtn = /^[a-zA-Z0-9]{2,5}$/; //영문 소문자 2자리 ~ 5자리 사이에 값을 비교하는 패턴
			
			var idPtn = /^[a-zA-Z]{1}[a-zA-z0-9] {1,4}$/; 
			//첫번째 글자는 영문 소문자,영문 대문자 1자리 나머지는 2~5자리 사이에 값을 비교하는 패턴
			
			//var namePtn = /^[가-힣]{2,5}$/; //영문 소문자 2자리 ~ 5자리 사이에 값을 비교하는 패턴
		//	var pwPtn = /^(?=.*[a-zA-Z])(?=.*[0-9])(?=.*[!@#$%^&*]).{3,} $/; //영문 소문자 2자리 ~ 5자리 사이에 값을 비교하는 패턴
		//	var f_tell = /^[0-9]{2,3}$/;
		//	var m_tell = /^[0-9]{3}$/;
			if(!idPtn.test(id)){
				//alert("영문,숫자 조합으로 2~5자리까지 입력이 가능합니다.");
				return false;
		...
		<section>
			<form name="agree" method="get" action="join03_success.html">
				...
				<div id="info_input_button">
					...
					<input type="button" id="minsertBtn" value="가입하기" />
				</div>
